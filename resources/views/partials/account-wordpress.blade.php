                        ...
                        <div class="border border-gray-200 rounded-xl p-5 hover:border-blue-200 transition-colors">
                            ...
                            <template x-if="(wpCredentials[wp.path]?.admin_users || []).find(u => u.is_default_login && u.stored_password)">
                                <div class="flex flex-wrap items-center gap-2 mb-3 text-xs">
                                    ...
                                    <button x-data="{done:false}" @click="var du=(wpCredentials[wp.path]?.admin_users||[]).find(u=>u.is_default_login);if(du){navigator.clipboard.writeText(du.stored_password).then(()=>{done=true;setTimeout(()=>{done=false},2000)})}"
                                                    ...
                                                    <td class="px-3 py-2">
                                                        <template x-if="user.stored_password">
                                                            ...
                                                            <div class="flex items-center gap-1.5 mt-1">
                                                                <button x-data="{done:false}" @click="navigator.clipboard.writeText(user.stored_password).then(()=>{done=true;setTimeout(()=>{done=false},2000)})"
