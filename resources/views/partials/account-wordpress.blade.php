                        ...
                        <div class="border border-gray-200 rounded-xl p-5 hover:border-blue-200 transition-colors">
                            ...
                            <template x-if="(wpCredentials[wp.path]?.admin_users || []).find(u => u.is_default_login && u.stored_password)">
                                <div class="flex flex-wrap items-center gap-2 mb-3 text-xs">
                                    ...
                                    <button x-data="{done:false}" @click="var du=(wpCredentials[wp.path]?.admin_users||[]).find(u=>u.is_default_login);if(du){$dispatch('copy-text',{text:du.stored_password,label:'Password'});done=true;setTimeout(()=>done=false,2000)}"
                                        class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded border transition-colors"
                                        :class="done ? 'text-green-700 bg-green-50 border-green-300' : 'text-blue-600 bg-blue-50 border-blue-200 hover:bg-blue-100'">
                                        <svg x-show="!done" class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                        <svg x-show="done" class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                        <span x-text="done ? 'Password Copied!' : 'Copy Password'"></span>
                                    </button>
                                    <button x-data="{done:false}" @click="var du=(wpCredentials[wp.path]?.admin_users||[]).find(u=>u.is_default_login);if(du){var info='Login URL: '+(wp.login_url||(wp.url+'/wp-login.php'))+'\nUsername: '+(du.user_login||du.username)+'\nPassword: '+du.stored_password;$dispatch('copy-text',{text:info,label:'Login Info'});done=true;setTimeout(()=>done=false,2000)}"
                                        class="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded border transition-colors"
                                        :class="done ? 'text-green-700 bg-green-50 border-green-300' : 'text-purple-600 bg-purple-50 border-purple-200 hover:bg-purple-100'">
                                        <svg x-show="!done" class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                                        <svg x-show="done" class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                        <span x-text="done ? 'Login Info Copied!' : 'Copy Login Info'"></span>
                                    </button>
                                </div>
                            </template>
                                                    ...
                                                    <td class="px-3 py-2">
                                                        <template x-if="user.stored_password">
                                                            ...
                                                            <div class="flex items-center gap-1.5 mt-1">
                                                                <button x-data="{done:false}" @click="$dispatch('copy-text',{text:user.stored_password,label:'Password'});done=true;setTimeout(()=>done=false,2000)"
                                                                    class="inline-flex items-center gap-0.5 px-1.5 py-0.5 text-[10px] font-medium rounded border transition-colors"
                                                                    :class="done ? 'text-green-700 bg-green-50 border-green-300' : 'text-gray-500 bg-gray-50 border-gray-200 hover:bg-gray-100'">
                                                                    <svg x-show="!done" class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                                                    <svg x-show="done" class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                                                    <span x-text="done ? 'Copied!' : 'Copy PW'"></span>
                                                                </button>
                                                                <button x-data="{done:false}" @click="var info='Login URL: '+(wp.login_url||(wp.url+'/wp-login.php'))+'\nUsername: '+(user.user_login||user.username)+'\nPassword: '+user.stored_password;$dispatch('copy-text',{text:info,label:'Login Info'});done=true;setTimeout(()=>done=false,2000)"
                                                                    class="inline-flex items-center gap-0.5 px-1.5 py-0.5 text-[10px] font-medium rounded border transition-colors"
                                                                    :class="done ? 'text-green-700 bg-green-50 border-green-300' : 'text-gray-500 bg-gray-50 border-gray-200 hover:bg-gray-100'">
                                                                    <svg x-show="!done" class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                                                                    <svg x-show="done" class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                                                    <span x-text="done ? 'Copied!' : 'Copy Login'"></span>
                                                                </button>
                                                            </div>
                                                        </template>
                                                        <template x-if="!user.stored_password">
