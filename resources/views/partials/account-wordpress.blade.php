                            ...
                            <div class="flex items-center gap-3 mb-3 text-xs text-gray-500">
                                <template x-if="wp.admin_user || wpCredentials[wp.path]?.stored_credentials?.username">
                                    <span class="inline-flex items-center gap-1">
                                        ...
                                        <span x-text="wp.admin_user || wpCredentials[wp.path]?.stored_credentials?.username"></span>
                                    </span>
                                </template>
                                                        ...
                                                        <template x-if="user.stored_password">
                                                            <div class="flex items-center gap-1">
                                                                ...
                                                                <button x-data="{copied:false}" @click="wpCopyText(user.stored_password);copied=true;setTimeout(()=>copied=false,1500)"
                                                                    class="shrink-0 p-0.5 transition-colors" :class="copied?'text-green-500':'text-gray-400 hover:text-green-600'" title="Copy password only">
                                                                ...
                                                                <button x-data="{copied:false}" @click="wpCopyLoginInfo(wp, user);copied=true;setTimeout(()=>copied=false,1500)"
                                                                    class="shrink-0 p-0.5 transition-colors" :class="copied?'text-green-500':'text-gray-400 hover:text-green-600'" title="Copy full login (URL + username + password)">
                                                                    <svg x-show="!copied" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                                                                    <svg x-show="copied" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                                                </button>
                                                            </div>
                                                        </template>
